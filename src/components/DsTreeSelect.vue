...
import type { DsTreeFilterNodeMethod, DsTreePropsMap } from '@/components/tree/dsTreeProps'
import type { DsTreeSelectModelValue, DsTreeSelectValueDisplay } from '@/components/dsTreeSelectTypes'
...
const resolvedFilterPlaceholder = computed(() => {
  return props.filterPlaceholder ?? t('fintDs.treeSelect.filterPlaceholder', 'Search…')
})
  ...
  >
    <div class="relative">
      ...
      <button
        ...
        :disabled="props.disabled"
        :aria-label="t('fintDs.common.clear', 'Clear')"
        @click.stop="clear"
      >
        ...
        <div :class="panelClasses">
          ...
          >
            <div v-if="(props.data?.length ?? 0) === 0" class="px-3 py-2 text-[13px] text-[var(--muted-foreground)]">
              <slot name="empty">
{{ t('fintDs.treeSelect.empty', 'No data') }}
</slot>
            </div>
