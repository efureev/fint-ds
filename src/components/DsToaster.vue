...
<template>
  <teleport to="body">
    <div class="fixed z-50 right-4 top-4 w-[360px] max-w-[calc(100vw-2rem)] grid gap-3">
      <div
        v-for="toast in items"
        :key="toast.id"
        class="rounded-[var(--ds-radius-lg)] border border-[var(--border)] bg-[var(--card)] shadow-[var(--ds-shadow-2)] px-4 py-3"
      >
        <div class="flex items-start gap-3">
          <component :is="iconFor(toast.variant)" class="h-5 w-5 mt-0.5" :style="{ color: colorFor(toast.variant) }" aria-hidden="true" />
          <div class="min-w-0 flex-1">
            <div class="text-[13px] font-700">
              {{ toast.title }}
            </div>
            <div v-if="toast.message" class="text-[13px] ds-muted mt-0.5">
              {{ toast.message }}
            </div>
          </div>
          <DsButton
