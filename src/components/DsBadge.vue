...
const props = withDefaults(
  defineProps<{
    ...
    /** When `true`, renders a fully filled badge (solid background). */
    dark?: boolean
...
const className = computed(() => {
  ...
  const radiusClass = (() => {
    if (props.radius === 'square') return 'rounded-[var(--ds-radius-none)]'
    if (props.radius === 'semi') return SEMI_RADIUS_BY_SIZE[props.size]
    return 'rounded-full'
  })()
