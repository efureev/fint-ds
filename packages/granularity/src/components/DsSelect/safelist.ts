...
const selectBase = [
  ...
  'relative w-full',
  'absolute top-1/2 -translate-y-1/2 right-3 text-[var(--muted-foreground)] pointer-events-none',
  'min-w-0 flex-1',
  'block truncate',
  'shrink-0 h-4 w-4',
  'shrink-0 text-[var(--muted-foreground)] pointer-events-none',
  'absolute top-1/2 -translate-y-1/2 right-3 h-6 w-6 inline-flex items-center justify-center rounded-md text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-[color-mix(in_srgb,var(--muted)_25%,transparent)] disabled:opacity-50',
  'absolute z-50 mt-2 w-full',
  'p-2 border-b border-[var(--border)]',
  'p-1 overflow-auto',
...
const selectVariants = [
  ...
  'text-[var(--foreground)] hover:text-[var(--primary)] active:text-[var(--primary-active)]',
  'text-[var(--muted-foreground)] hover:text-[var(--foreground)] active:text-[var(--foreground)]',
