...
describe('granularity theming and node runtime', () => {
  ...
  it('собирает node-only theme и component preflights через ./uno-node', async () => {
    ...
    expect(buttonCss).toContain('.bg-\\[var\\(--primary\\)\\]')
    ...
    expect(iconCss).toContain('.inline-flex')
    expect(iconCss).toContain('.items-center')
    expect(iconCss).toContain('.justify-center')
    expect(iconCss).toContain('.align-middle')
  ...
  it('читает theme css из node-only helper-ов и поддерживает пользовательские файлы', async () => {
    ...
    expect(normalizedDefaultTokensCss).toContain('--primary-hover:color-mix(insrgb,var(--primary)92%,var(--foreground))')
    expect(normalizedDefaultTokensCss).not.toContain('--background:')
    ...
    expect(normalizedDarkThemeCss).toContain('--background:#0f172a')
    expect(normalizedDarkThemeCss).not.toContain(':root{--background:#f8fafc')
    expect(normalizedCombinedThemeCss).toContain("[data-theme='dark']")
    expect(normalizedCombinedThemeCss).toContain('--background:#0f172a')
    expect(normalizedCombinedThemeCss).toContain('--background:#f8fafc')
  })
