...
<template>
  <div class="grid gap-3">
    <DsButton class="justify-self-start" @click="loading = !loading">
    ...
    <div class="relative min-h-[180px] rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
      <div class="grid gap-2 text-sm text-[var(--muted-foreground)]">
        <div class="font-medium text-[var(--foreground)]">Invoice list</div>
        <div>Use `DsLoading` as an overlay above an existing card or section while async data is refreshing.</div>
      </div>
